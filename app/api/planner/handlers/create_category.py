...
from app.schemas.category import CategoryCreate, CategorySchema  # Предполагается, что у вас есть такая схема Pydantic
...
async def create_category_handler(data: CategoryCreate) -> Response:
    ...
    async with get_async_session() as session:
        if await check_category_by_name(data.name, session):
            return jsonify({"message": f"Category with name: '{data.name}' already exists!"})
