...
async def get_all_expenses_handler(user_id: int, account_id: Optional[int] = None) -> tuple[Response, int]:
    async with get_async_session() as session:
        if not account_id:
            ...
            result = await session.execute(
                select(Expense).options(joinedload(Expense.category), joinedload(Expense.account)).where(
                    Expense.user_id == user_id, Expense.account_id == account_id
                ).order_by(desc(Expense.date))
            )
